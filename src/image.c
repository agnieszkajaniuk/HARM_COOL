...
{ 
  ...
  ************************************************************************/
  ...
  IMAGELOOP { 
    ...
    if( gamma_calc(p[h][i][j],&geom,&gamma) ) { gamma = 1.; }
    
    fimage[IRHO][k] = p[h][i][j][RHO] ; 
    fimage[IUU ][k] = p[h][i][j][UU ] ;
    fimage[IBSQ][k] = bsq_calc( p[h][i][j], &geom ) ; 
    fimage[IGAM][k] = gamma ;
    k++;
  }
  ...
  ************************************************************************/
  for( i_img = 0 ; i_img < NIMG; i_img++ ) {
	  for( i = 0 ; i < /*N1*N2*/(istop-istart+1)*(jstop-jstart+1); i++ ) { 
    	fimage[i_img][i] = log( fabs(fimage[i_img][i]) + fimage_logmin );
	  }
  }
...
void image_r8(double *f, char *fname)
{
  ...
  /* Header information: */
  fprintf(fp, "RAW\n#  min=%g  , max=%g \n%d %d\n%d\n", min, max, /*N1*/(istop-istart+1), /*N2*/(jstop-jstart+1), (NCOLORS-1) );
  fflush(fp);
...
void image_ppm(double *f, char *fname)
{
  ...
  /* Header information: */
  fprintf(fp, "P6\n#  min=%g  , max=%g \n%d %d\n%d\n", min, max, /*N1*/(istop-istart+1), /*N2*/(jstop-jstart+1), ppm_ncolors );
  fflush(fp);
