...
void primtoU_g( FTYPE prim[], FTYPE gcov[NDIM][NDIM], FTYPE gcon[NDIM][NDIM], FTYPE gdet, 
		FTYPE U[] );
void ucon_calc_g(FTYPE prim[],FTYPE gcov[NDIM][NDIM],FTYPE gcon[][4],FTYPE ucon[]);
void raise_g(FTYPE vcov[], FTYPE gcon[NDIM][NDIM], FTYPE vcon[]);
void lower_g(FTYPE vcon[], FTYPE gcov[NDIM][NDIM], FTYPE vcov[]);
void ncov_calc(FTYPE gcon[NDIM][NDIM],FTYPE ncov[]) ;
...
void primtoU_g(
  ...
  u = prim[UU] ;
#if( COOL_EOS )
  p = pressure_rho0_u_CoolEOS(rho0,u) ;
#else
