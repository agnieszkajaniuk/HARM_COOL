...
void primtoflux(double *pr, struct of_state *q, const int dir, 
		struct of_geom *geom, double *flux) 
{
...
#if( COOL_EOS )
	{
		double rho = pr[RHO];
		double u = pr[UU];
		double p = pressure_rho0_u_CoolEOS(rho,u);
		mhd_calc_CoolEOS(rho, u, p, dir, q, mhd) ;
	}
#else
...
void source(double *ph, struct of_geom *geom, const int hh, const int ii, const int jj, double *dU,
		double Dt)
{
...
#if( COOL_EOS )
	{
		...
		mhd_calc_CoolEOS(rho, u, p, 0, &q, mhd[0]) ;
		mhd_calc_CoolEOS(rho, u, p, 1, &q, mhd[1]) ;
		mhd_calc_CoolEOS(rho, u, p, 2, &q, mhd[2]) ;
		mhd_calc_CoolEOS(rho, u, p, 3, &q, mhd[3]) ;
	}
#else
...
int gamma_calc(double *pr, struct of_geom *geom, double *gamma)
{
        ...
        if( qsq < 0. ){
          if( fabs(qsq) > 1.E-10 ){ // then assume not just machine precision
            fprintf(stderr,"gamma_calc():  failed: k,i,j,qsq = %d %d %d %28.18e \n", hcurr, icurr,jcurr,qsq);
		...
		double *vmax, double *vmin)
{
	...
	#if (COOL_EOS_SIMPLECS)
		cs2 = p/(rho+u);
	#else
		// On the convexity of relativistic hydrodynamics
		// Jose M Ibanez, Isabel Cordero-Carrion, Jose M Marti and Juan A Miralles
		{
			double e = u/rho; //specific energy
			double enthalpy = 1.0 + e + p/rho;
			double dpdrho = dpdrho_calc_CoolEOS(rho, u);
			double dpde   = dpdu_calc_CoolEOS(rho, u)*rho;
			                      
			cs2 = (dpdrho + p/rho/rho*dpde)/enthalpy; 
    	}
	#endif
...
void misc_source(double *ph, int hh, int ii, int jj, struct of_geom *geom, 
