...
!--------------------------------------------------------------
      ...
      subroutine gauss_laguerre(f,a,b,result,par,n)
...
!declare
      ...
      external         f
      integer          j,n
      double precision a,b,result,par(n),absc,wg(20),xg(20),fval
...
!---------------------------------------------------------
      ...
      subroutine gauss_legendre(f,a,b,fresult,par,n)
...
!declare 
      external         f
      integer          j,n
      double precision a,b,fresult,dresult,ddresult,par(n), &
...
!       print*,'-------------------------'
      do j=1,20 
        ...
        fresult   = fresult + (fval1 + fval2)*wg(j)
!       print*,'n,par(6)=',n,par(6)
!       print*,'absc1,fval1=',absc1,fval1
!       print*,'result=',fresult
      enddo
      fresult   = fresult * hlfrun
      return
...
! abscissae and weights were evaluated with 100 decimal digit arithmetic.
      ...
      data wg (  2) /   1.4917298647260374678728737001969436d-1 /
...
! abscissae and weights were evaluated with 100 decimal digit arithmetic.
      ...
      data wg (  4) /   9.15352372783073672670604684771868067d-1 /
      data wg (  5) /   1.16953970719554597380147822239577476d0 /
      data wg (  6) /   1.4313549859282059863684499489154331d0 /
      data wg (  7) /   1.70298113798502272402533261633206720d0 /
      data wg (  8) /   1.98701589079274721410921839275129020d0 /
      data wg (  9) /   2.28663578125343078546222854681495651d0 /
      data wg ( 10) /   2.60583472755383333269498950954033323d0 /
